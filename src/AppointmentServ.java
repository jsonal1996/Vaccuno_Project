

import java.io.IOException;
import javax.servlet.ServletException;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import javax.servlet.http.HttpSession;

/**
 * Servlet implementation class AppointmentServ
 */
public class AppointmentServ extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AppointmentServ() {
        super();
        // TODO Auto-generated constructor stub
    }

	/**
	 * @see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		response.getWriter().append("Served at: ").append(request.getContextPath());
	}

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		doGet(request, response);
		HttpSession session=request.getSession();
		String usename = (String) session.getAttribute("uname");
		String dname = request.getParameter("check");
		
		String name = (String)session.getAttribute("name");
		String cost =(String)session.getAttribute("cost");
		String vendor = (String)session.getAttribute("vendor");
		String disease = (String)session.getAttribute("disease");
		String minNoDoses =(String)session.getAttribute("minNoDoses");
		

		Appointment a=new Appointment();
		a.addAppointment(dname, usename, name, cost, vendor, disease, minNoDoses);
		System.out.println("Appointment added successfully");
		response.sendRedirect("VaccinelistCustServ");
		return;
	}

}
